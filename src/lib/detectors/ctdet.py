...
class CtdetDetector(BaseDetector):
    ...
    def process(self, images, anno, return_time=False):
        with torch.no_grad():
            output = self.model(images)[-1]
            hm = output['hm']  # 在inference过程中model内进行sigmoid
            wh = output['wh']
            ...
            dets = ctdet_decode(hm, wh, reg=reg, cat_spec_wh=self.opt.cat_spec_wh, K=self.opt.K) # [1,100,6], x1,y1,x2,y2,conf,cat
    ...
    def show_results(self, debugger, image, results, img_name):
        ...
        img_name = debugger.show_all_imgs(pause=self.pause, img_name=img_name)
