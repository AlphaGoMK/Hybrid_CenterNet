...
class Debugger(object):
    ...
    def show_all_imgs(self, img_name, sub_folder='', pause=False, time=0):
        if not self.ipynb:
            if sub_folder != '':
                cv2.imwrite('demo/%s/det_%s.jpg' % (sub_folder, img_name.split('/')[-1].split('.')[0]), self.imgs['ctdet'])
            else:
                cv2.imwrite('demo/det_%s.jpg' % img_name.split('/')[-1].split('.')[0], self.imgs['ctdet'])
        else:
            self.ax = None
            nImgs = len(self.imgs)
        ...
        if sub_folder != '':
            return 'demo/%s/det_%s.jpg' % (sub_folder, img_name.split('/')[-1].split('.')[0])
        else:
            return 'demo/det_%s.jpg' % img_name.split('/')[-1].split('.')[0]
